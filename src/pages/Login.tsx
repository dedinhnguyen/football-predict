...
import { Trophy, ShieldAlert, LogIn, Sun, Moon, Globe, User } from 'lucide-react';
    ...
    <div className="flex min-h-screen w-screen flex-col">
      ...
      <div className="relative flex flex-1 items-center justify-center p-4">
        ...
        <div className="glass-panel w-full max-w-md rounded-2xl p-6 sm:p-8 text-center shadow-2xl relative overflow-hidden transition-all duration-500">
          ...
          {/* Divider */}
          <div className="my-5 flex items-center justify-between text-slate-500">
            <span className="w-1/4 border-b border-white/5" />
            <span className="text-[10px] uppercase tracking-wider font-semibold">{t('or')}</span>
            <span className="w-1/4 border-b border-white/5" />
          </div>
          ...
          <button
            ...
            disabled={isLoggingIn}
            className="group relative flex w-full items-center justify-center gap-2.5 overflow-hidden rounded-xl border border-white/10 bg-white/5 px-5 py-3.5 text-sm font-semibold text-slate-300 transition-all duration-300 hover:bg-white/10 hover:text-white hover:border-white/20 active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50"
          >
            <User className="h-4.5 w-4.5 text-slate-400 group-hover:text-slate-200 transition-colors" />
            <span>{t('loginGuestBtn')}</span>
          </button>
